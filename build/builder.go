...
/*
	creates a brandnew Morphine compiled build set for client
...
func main() {
	...
	target := "Morphine-" + *target_username + "@" + *target_version
	...
	if err := os.RemoveAll(*GoRoot + "/src/Morphine"); err != nil {
		fmt.Printf("\x1b[34m[6/11]\x1b[0m: error when removing %s: %s\r\n", *GoRoot+"/src/Morphine", err.Error())
	} else {
		fmt.Printf("\x1b[34m[6/11]\x1b[0m: removed %s\r\n", *GoRoot+"/src/Morphine")
	}

	//creates the dir properly and safely
	if err := os.Mkdir(*GoRoot+"/src/Morphine", os.ModePerm); err != nil {
	...
	if err := copy(*GoPath+"/src/Morphine", *GoRoot+"/src/Morphine"); err != nil {
		fmt.Printf("\x1b[34m[8/11]\x1b[0m: error when coping current outlook to %s -> %s: %s\r\n", *GoPath+"/src/Morphine", *GoRoot+"/src/Morphine", err.Error()) //return
	} else {
		fmt.Printf("\x1b[34m[8/11]\x1b[0m: moved %s -> %s\r\n", *GoPath+"/src/Morphine", *GoRoot+"/src/Morphine")
	}

	//moves the cursor properly
	if err := os.Chdir("/" + *GoPath + "/src/Morphine/compression"); err != nil {
		fmt.Printf("\x1b[34m[9/11]\x1b[0m: error when moving current outlook to %s: %s\r\n", *GoPath+"/src/Morphine", err.Error()) //return
	} else {
		fmt.Printf("\x1b[34m[9/11]\x1b[0m: moved current outlook to %s\r\n", *GoPath+"/src/Morphine")
	}

	//copies the information properly
	if err := copy("/"+*GoPath+"/src/Morphine/compression/garble", "/"+*GoPath+"/src/Morphine"); err != nil {
		fmt.Printf("\x1b[34m[10/11]\x1b[0m: error when coping current outlook to %s -> %s: %s\r\n", "/"+*GoPath+"/src/Morphine/compression/garble", "/"+*GoPath+"/src/Morphine", err.Error()) //return
	} else {
		fmt.Printf("\x1b[34m[10/11]\x1b[0m: moved %s -> %s\r\n", "/"+*GoPath+"/src/Morphine/compression/garble", "/"+*GoPath+"/src/Morphine")
	}

	//moves the cursor properly
	if err := os.Chdir("/" + *GoPath + "/src/Morphine"); err != nil {
	...
	fmt.Printf("         Run:\r\n")
	fmt.Printf("           > cd Morphine")
	fmt.Printf("           > ./garble -literals -debug -tiny -seed=random build -ldflags=\"-w -s\" Morphine\r\n")
	fmt.Printf("           > strip Morphine\r\n")
	fmt.Printf("           > upx -9 -1 Morphine\r\n")
}
